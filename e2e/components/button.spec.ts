...
test.describe('Syncfusion Button Components', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await expect(page.getByTestId(TestIds.BTN_PRIMARY)).toBeVisible({ timeout: 30000 });
  });
  ...
  test('should have disabled button that cannot be clicked', async ({ page }) => {
    ...
    // Syncfusion ButtonComponent with disabled={true} sets the HTML disabled attribute
  ...
  test('should have proper button focus states', async ({ page }) => {
    ...
    // Verify the button is still visible after focus
