...
// Run with: npx playwright test e2e/components/menu-syncfusion
test.describe('Syncfusion Menu Components', () => {
  test.setTimeout(180000);

  test.beforeEach(async ({ page }) => {
    await injectAuth(page);
    await page.goto('/components/menu/syncfusion');
  ...
  test('should navigate to correct URL', async ({ page }) => {
    expect(page.url()).toContain('/components/menu/syncfusion');
  });
});
