 ...
 * the current route. For example, `/components/button/native`
...
export class HeaderBreadcrumbPage extends BasePage {
  ...
  /**
   * Get all breadcrumb segment labels (clickable links and current page span).
  ...
  get segments(): Locator {
    return this.breadcrumbNav.locator(
      `a:not([data-testid="${TestIds.HEADER_BREADCRUMB_HOME}"]), span[aria-current="page"]:not([data-testid="${TestIds.HEADER_BREADCRUMB_HOME}"])`
    );
  }
  ...
  /**
   * Get all clickable breadcrumb segments (links, not the current page span).
   * Excludes the home link.
   */
  get clickableSegments(): Locator {
    return this.breadcrumbNav.locator(
      `a:not([data-testid="${TestIds.HEADER_BREADCRUMB_HOME}"])`
    );
  }
  ...
  async clickSegment(text: string): Promise<void> {
    await this.breadcrumbNav.locator('a', { hasText: text }).first().click();
  }
  ...
  async expectVisible(): Promise<void> {
    await expect(this.breadcrumbNav).toBeVisible({ timeout: 15000 });
  }
