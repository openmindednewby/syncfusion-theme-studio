...
const enforceLazyPreloadRule = {
  meta: {
    ...
    messages: {
      missingPreload:
        'Lazy route "{{path}}" has no preload in {{fn}}(). Without preloading, first navigation triggers a loading spinner. Add: import(\'{{path}}\').catch(() => undefined);',
      missingPreloadFn:
        'File has {{count}} lazy() route(s) but no {{fn}}() function. Users will see loading spinners on first navigation. Add a preload function.',
      stalePreload:
        'Preload "{{path}}" in {{fn}}() has no matching lazy() route. This loads unused code. Remove it or add the corresponding lazy route.',
    },
  },
