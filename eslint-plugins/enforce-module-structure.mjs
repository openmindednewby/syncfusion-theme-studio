...
const enforceModuleStructureRule = {
  meta: {
    ...
    messages: {
      hookAtRoot:
        'Hook "{{fileName}}" at module root. Hooks in a flat directory are hard to find as the module grows. Move to {{suggestedPath}}.',
      componentAtRoot:
        'Sub-component "{{fileName}}" at module root. Grouping sub-components improves discoverability. Move to {{suggestedPath}}.',
      utilAtRoot:
        'Utility "{{fileName}}" at module root. Separating utils from components makes responsibilities clear. Move to {{suggestedPath}}.',
      dataInSections:
        'Data file "{{fileName}}" mixed with section components. Data and UI should be separated for clarity. Move to {{suggestedPath}}.',
    },
  },
