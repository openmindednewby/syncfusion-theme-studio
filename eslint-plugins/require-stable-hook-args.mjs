...
const requireStableHookArgsRule = {
  meta: {
    ...
    messages: {
      unstableFunction:
        'Unstable function "{{name}}" passed to "{{hook}}". New reference every render → infinite re-render loop. Wrap in useCallback().',
      unstableObject:
        'Unstable object "{{name}}" passed to "{{hook}}". New reference every render → infinite re-render loop. Wrap in useMemo().',
      unstableArray:
        'Unstable array "{{name}}" passed to "{{hook}}". New reference every render → infinite re-render loop. Wrap in useMemo().',
      unstableInlineFunction:
        'Inline function passed to "{{hook}}". New reference every render → infinite re-render loop. Extract to a useCallback() variable.',
      unstableInlineObject:
        'Inline object passed to "{{hook}}". New reference every render → infinite re-render loop. Extract to a useMemo() variable.',
      unstableInlineArray:
        'Inline array passed to "{{hook}}". New reference every render → infinite re-render loop. Extract to a useMemo() variable.',
    },
  },
