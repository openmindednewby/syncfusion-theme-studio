...
function main() {
  ...
  for (const result of results) {
    ...
    const scoreText = `${result.score}%`.padEnd(5);
    const score = `${result.color}${scoreText}${RESET}`;
    const threshold = `${result.threshold}%`.padEnd(9);
    const statusText = result.passed ? 'PASS' : 'FAIL';
    const statusColor = result.passed ? GREEN : RED;
    const status = `${statusColor}${statusText.padEnd(6)}${RESET}`;
    console.log(`│ ${name} │ ${score} │ ${threshold} │ ${status} │`);
  }
