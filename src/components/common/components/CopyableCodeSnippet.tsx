      ...
      <div className="overflow-hidden rounded-lg border border-code-border bg-code-bg">
        <div className="flex">
          ...
          <div className="relative flex-1">
            <pre className="overflow-x-auto py-3 pr-10 font-mono text-xs leading-5">
              <code>
                {lineEntries.map((entry) => (
                  <div key={entry.id}>
                    {entry.tokens.length > 0
                      ? entry.tokens.map((token, ti) => (
                          // eslint-disable-next-line react/no-array-index-key -- tokens lack unique IDs; array is never reordered
            ...
            {/* Scroll affordance: fade gradient on mobile to hint at horizontal overflow */}
            <div
              aria-hidden="true"
              className="pointer-events-none absolute inset-y-0 right-0 w-8 bg-gradient-to-l from-code-bg to-transparent md:hidden"
            />
          </div>
        </div>
      </div>
