  ...
  return (
    <li>
      <button
        ...
        className={`sidebar-item flex w-full items-center gap-3 px-3 py-1.5 text-sm transition-colors ${
      ...
      <div className={`sidebar-expandable-children ${effectiveExpanded ? 'expanded' : 'collapsed'}`}>
        <ul className="ml-8 mt-1 space-y-0.5" id={`subgroup-${testId}`}>
          {items.map((item) => (
            <li key={item.path}>
              <NavLink
                aria-label={FM(item.labelKey)}
                className={({ isActive: childActive }) =>
                  `sidebar-item flex items-center gap-2 px-3 py-1 text-xs transition-colors ${
                    childActive ? 'active' : ''
                  }`
                }
