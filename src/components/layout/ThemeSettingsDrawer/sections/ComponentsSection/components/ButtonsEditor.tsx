...
const VariantEditor = ({ config, onUpdate, variant }: VariantEditorProps): JSX.Element => (
  <div className="space-y-2">
    ...
    <div className="grid grid-cols-2 gap-2 pl-2">
      ...
      <ColorPicker label={L('textColorActive')} value={config.textColorActive} onChange={(v) => onUpdate({ textColorActive: v })} />
      <ColorPicker label={L('borderColor')} value={config.borderColor} onChange={(v) => onUpdate({ borderColor: v })} />
      <ColorPicker label={L('borderColorActive')} value={config.borderColorActive} onChange={(v) => onUpdate({ borderColorActive: v })} />
