...
export const DataGridEditSelectionEditor = ({
  ...
  <CollapsibleSection title={FM('themeSettings.components.dataGrid.editSelectionSection')}>
    <div className="grid grid-cols-2 gap-2">
      ...
      />
      <ColorPicker
        label={FM('themeSettings.components.dataGrid.selectionBarChipBackground')}
        value={config.selectionBarChipBackground}
        onChange={(value) => onUpdate({ selectionBarChipBackground: value })}
      />
      <ColorPicker
        label={FM('themeSettings.components.dataGrid.selectionBarClearBackground')}
        value={config.selectionBarClearBackground}
        onChange={(value) => onUpdate({ selectionBarClearBackground: value })}
      />
      <ColorPicker
        label={FM('themeSettings.components.dataGrid.selectionBarClearTextColor')}
        value={config.selectionBarClearTextColor}
        onChange={(value) => onUpdate({ selectionBarClearTextColor: value })}
      />
    </div>
  </CollapsibleSection>
);
