...
export const DataGridGroupAggregateEditor = ({
  ...
  <CollapsibleSection title={FM('themeSettings.components.dataGrid.groupAggregateSection')}>
    <div className="grid grid-cols-2 gap-2">
      ...
      />
      <ColorPicker
        label={FM('themeSettings.components.dataGrid.groupDropAreaTextColor')}
        value={config.groupDropAreaTextColor}
        onChange={(value) => onUpdate({ groupDropAreaTextColor: value })}
      />
      <ColorPicker
        label={FM('themeSettings.components.dataGrid.groupDropAreaBorderColor')}
        value={config.groupDropAreaBorderColor}
        onChange={(value) => onUpdate({ groupDropAreaBorderColor: value })}
      />
      <ColorPicker
        label={FM('themeSettings.components.dataGrid.groupChipBackground')}
        value={config.groupChipBackground}
        onChange={(value) => onUpdate({ groupChipBackground: value })}
      />
      <ColorPicker
        label={FM('themeSettings.components.dataGrid.groupChipTextColor')}
        value={config.groupChipTextColor}
        onChange={(value) => onUpdate({ groupChipTextColor: value })}
      />
      <ColorPicker
