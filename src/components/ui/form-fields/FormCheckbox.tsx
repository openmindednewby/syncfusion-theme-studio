...
interface FormCheckboxProps<T extends FieldValues>
  extends Omit<CheckboxNativeProps, 'checked' | 'onChange' | 'error' | 'value'> {
  ...
  helperText?: string;
  /** Value for checkbox group (array fields). When provided, toggles this value in/out of the array. */
  value?: string;
}
...
export const FormCheckbox = <T extends FieldValues>({
  ...
  <Controller
    ...
    render={({ field, fieldState }) => {
      ...
      const arrayValues: string[] = isArrayField ? field.value : [];

      function handleChange(e: ChangeEvent<HTMLInputElement>): void {
        if (isArrayField && isValueDefined(groupValue)) {
          const next = e.target.checked
            ? [...arrayValues, groupValue]
            : arrayValues.filter((v) => v !== groupValue);
          field.onChange(next);
        } else
          field.onChange(e.target.checked);
      }

      const isChecked = isArrayField && isValueDefined(groupValue)
        ? arrayValues.includes(groupValue)
        : field.value === true;
