 ...
 */
import React, { memo, forwardRef, useId, useRef, useCallback, useMemo, useEffect } from 'react';
...
const SelectNative = forwardRef<HTMLDivElement, SelectNativeProps>(
  (
    ...
    const sizeStyle = useMemo((): React.CSSProperties | undefined => {
      ...
      if (isValueDefined(width)) s.width = typeof width === 'number' ? `${width}px` : width;
      if (isValueDefined(minWidth)) s.minWidth = typeof minWidth === 'number' ? `${minWidth}px` : minWidth;
      if (isValueDefined(maxWidth)) s.maxWidth = typeof maxWidth === 'number' ? `${maxWidth}px` : maxWidth;
    ...
    const shouldApplyAriaLabel = !isValueDefined(label) && isValueDefined(ariaLabel) && ariaLabel !== '';
    const ariaLabelProps = shouldApplyAriaLabel ? { ariaLabel } : {};
