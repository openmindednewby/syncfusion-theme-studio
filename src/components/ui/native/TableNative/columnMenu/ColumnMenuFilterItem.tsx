  ...
  ({ field, columnType, filterValue, filterOperator, onFilterChange, onFilterOperatorChange }: ColumnMenuFilterItemProps): JSX.Element => {
    ...
    const handleOperatorChange = useCallback(
      (e: React.ChangeEvent<HTMLSelectElement>) => {
        const matched = operators.find((op) => String(op) === e.target.value);
        if (isValueDefined(matched)) onFilterOperatorChange(field, matched);
      },
      [field, operators, onFilterOperatorChange],
    );
