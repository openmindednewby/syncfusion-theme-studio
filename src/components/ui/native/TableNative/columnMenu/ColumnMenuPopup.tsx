...
function useClickOutsideMenu(
  ...
  useEffect(() => {
    const handler = (e: Event): void => {
      ...
      if (isValueDefined(popupRef.current) && popupRef.current.contains(e.target)) return;
      if (isValueDefined(triggerRef.current) && triggerRef.current.contains(e.target)) return;
      onCloseRef.current();
    };
