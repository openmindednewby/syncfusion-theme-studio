...
const CheckboxCell = memo(({
  ...
  <td style={CHECKBOX_CELL_STYLE}>
    <input
      aria-label={FM('table.selectRow', String(rowId))}
      checked={isSelected}
...
const CommandCellWrapper = memo(({
  cellPadding, allowEditing, allowDeleting, isEditingRow, rowId, editing, extra,
...
const DataCell = memo(({
  ...
  const isEditableCell = !isValueDefined(column.template) && isCellInEditMode(editCtx, column.field);
