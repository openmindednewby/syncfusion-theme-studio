...
describe('useTableSelection', () => {
  ...
  describe('onSelectionChange', () => {
    ...
    it('fires with checkbox mode and plain click (no modifier)', () => {
      const onSelectionChange = vi.fn();
      const { result } = renderHook(() =>
        useTableSelection({
          data: MOCK_DATA,
    ...
    it('fires with paginated data subset', () => {
      const pageData = MOCK_DATA.slice(0, 3);
      const onSelectionChange = vi.fn();
      const { result } = renderHook(() =>
        useTableSelection({
          data: pageData,
      ...
      act(() => {
        result.current.handleRowClick(pageData[1]!, 1, NO_MODIFIERS);
      });

      expect(onSelectionChange).toHaveBeenCalledTimes(1);
      expect(onSelectionChange).toHaveBeenCalledWith([pageData[1]]);
    });

    it('accumulates multiple selections with checkbox mode', () => {
      const onSelectionChange = vi.fn();
      const { result } = renderHook(() =>
        useTableSelection({
          data: MOCK_DATA,
      ...
      act(() => {
        result.current.handleRowClick(MOCK_DATA[2]!, 2, NO_MODIFIERS);
      });

      expect(onSelectionChange).toHaveBeenCalledTimes(2);
      expect(onSelectionChange).toHaveBeenLastCalledWith([MOCK_DATA[0], MOCK_DATA[2]]);
    });
  });
});
