...
export type TabsNativeProps = BaseTabsProps;

const TabsNative = memo(({
  items,
  defaultActiveId,
  onChange,
  testId = 'tabs-native',
  orientation = 'horizontal',
}: TabsNativeProps): JSX.Element => {
  ...
  const handleKeyDown = useCallback((e: React.KeyboardEvent, id: string, _index: number) => {
  ...
  return (
    <div
      className={`${isVertical ? 'flex gap-4' : ''}`}
      data-testid={testId}
    >
      <div
        className={`flex ${isVertical ? 'flex-col border-r' : 'border-b'}`}
        role="tablist"
        style={{
          borderColor: 'var(--component-tabs-border)',
          backgroundColor: 'var(--component-tabs-bg)',
        }}
      >
        {items.map((item, index) => {
          ...
          return (
            <button
              ...
              className={`relative px-4 py-2.5 text-sm font-medium transition-colors focus:outline-none focus-visible:ring-2 ${
                item.disabled === true ? 'cursor-not-allowed opacity-50' : 'cursor-pointer'
              }`}
              data-tab-id={item.id}
              ...
              style={{
                color: isActive
                  ? 'var(--component-tabs-active-text)'
                  : 'var(--component-tabs-inactive-text)',
                backgroundColor: isActive
                  ? 'var(--component-tabs-active-bg)'
                  : 'transparent',
              }}
...
TabsNative.displayName = 'TabsNative';
