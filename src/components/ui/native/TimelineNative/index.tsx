...
/** Renders the timeline marker dot and connector line. */
const TimelineMarker = ({ item, index, isLast, isVertical }: MarkerProps): JSX.Element => {
...
/** Renders a single timeline step with marker, connector, and content. */
const TimelineItemRow = ({ item, index, isLast, isVertical }: ItemProps): JSX.Element => (
  <div
    className={`flex ${isVertical ? 'flex-row' : 'flex-col items-center'} ${
      isVertical ? '' : 'flex-1'
    }`}
  >
    <TimelineMarker index={index} isLast={isLast} isVertical={isVertical} item={item} />
