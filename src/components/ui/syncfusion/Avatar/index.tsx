...
export type SyncfusionAvatarProps = BaseAvatarProps;
...
function renderContent(props: AvatarContentProps): JSX.Element {
  const { showImage, src, alt, onImgError, icon, initials, sizeStyles } = props;

  if (showImage)
    return (
      <img alt={alt} className="h-full w-full object-cover" role="presentation" src={src} onError={onImgError} />
    );

  if (isValueDefined(icon)) return <span className="flex items-center justify-center">{icon}</span>;

  if (isNotEmptyString(initials)) return <span className={sizeStyles.text}>{initials}</span>;
...
const SyncfusionAvatar = memo(({
  ...
  icon,
  testId = 'sf-avatar',
}: SyncfusionAvatarProps): JSX.Element => {
  const [imgError, setImgError] = useState(false);
  const handleError = useCallback(() => setImgError(true), []);
  const hasValidSrc = isNotEmptyString(src) && !imgError;
  const sizeStyles = SIZE_MAP[size];

  return (
    <span className="relative inline-flex" data-testid={testId}>
      <span
        className={`e-avatar ${sizeStyles.container} inline-flex items-center justify-center overflow-hidden rounded-full border-2 font-medium`}
        style={{
          backgroundColor: hasValidSrc ? 'transparent' : 'var(--component-avatar-bg)',
          color: 'var(--component-avatar-text)',
          borderColor: 'var(--component-avatar-border)',
        }}
      >
        {renderContent({ showImage: hasValidSrc, src, alt, onImgError: handleError, icon, initials, sizeStyles })}
      </span>
      {showStatus ? <span
...
SyncfusionAvatar.displayName = 'SyncfusionAvatar';
export default SyncfusionAvatar;
