...
export function useColumnMenuEffect(
  ...
  useEffect(() => {
    const isActive = features.columnMenu || features.filtering || features.columnChooser;
    ...
    let columnChooserRafId: number | null = null;
    ...
    const scheduleFilterReposition = (anchor: DOMRect): void => {
      ...
      const tick = (): void => {
        attempt += 1;
        const positioned = repositionFilterPopup(anchor, tracking.openToLeft, tracking.gridId);
    ...
    const scheduleColumnChooserReposition = (): void => {
      let attempt = 0;
      let successCount = 0;
      const tick = (): void => {
        attempt += 1;
        const positioned = repositionColumnChooserPopup(root);
        if (positioned) successCount += 1;
        const done = attempt >= SUBMENU_REPOSITION_MAX_ATTEMPTS
          || (positioned && successCount >= SUBMENU_REPOSITION_MIN_FRAMES);
        if (done) { columnChooserRafId = null; return; }
        columnChooserRafId = requestAnimationFrame(tick);
      };
      if (isValueDefined(columnChooserRafId))
        cancelAnimationFrame(columnChooserRafId);
      columnChooserRafId = requestAnimationFrame(tick);
    };
    ...
    const onGridPointerDown = (event: Event): void => {
      ...
      // Column chooser toolbar button click — schedule with retry frames
      // so repositioning fires after Syncfusion finishes opening the dialog
      const ccTrigger = target.closest<HTMLElement>('.e-cc-toolbar');
      if (isValueDefined(ccTrigger)) {
        setTimeout(() => scheduleColumnChooserReposition(), POPUP_REPOSITION_DELAY_MS);
        scheduleColumnChooserReposition();
      }

      // Column menu header button click
      const trigger = target.closest<HTMLElement>('.e-columnmenu');
      if (!isValueDefined(trigger)) return;
      tracking.columnMenuTrigger = trigger;
      tracking.gridId = trigger.closest<HTMLElement>('.e-grid')?.id;
      setTimeout(() => {
    ...
    const onWindowMove = (): void => {
      ...
      if (isValueDefined(tracking.filterAnchor))
        repositionFilterPopup(tracking.filterAnchor, tracking.openToLeft, tracking.gridId);
      repositionColumnChooserPopup(root);
      scheduleDropdownReposition();
    ...
    const observer = new MutationObserver((mutations) => {
      ...
      let shouldRepositionChooser = false;
      ...
      for (const mutation of mutations) {
        ...
        if (attrResult.shouldRepositionChooser) shouldRepositionChooser = true;
      }

      if (shouldCleanup)
        setTimeout(() => cleanupZombies(), ZOMBIE_CLEANUP_LONG_DELAY_MS);
      if (shouldReposition) scheduleDropdownReposition();
      if (shouldRepositionChooser) scheduleColumnChooserReposition();
    });
    ...
    return () => {
      ...
      if (isValueDefined(submenuRafId)) cancelAnimationFrame(submenuRafId);
      if (isValueDefined(columnChooserRafId)) cancelAnimationFrame(columnChooserRafId);
    };
  }, [features.columnMenu, features.columnChooser, features.filtering, wrapperRef, trackingRef]);
}
...
function mutationChangedPopupAttribute(mutation: MutationRecord): {
  ...
  shouldRepositionChooser: boolean;
} {
  if (mutation.type !== 'attributes' || !(mutation.target instanceof HTMLElement))
    return { shouldReposition: false, shouldCleanup: false, shouldRepositionChooser: false };
  ...
  // Detect column chooser dialog gaining the e-popup-open class
  const shouldRepositionChooser =
    mutation.attributeName === 'class'
    && el.matches('.e-ccdlg.e-popup-open');
  return { shouldReposition, shouldCleanup, shouldRepositionChooser };
}
