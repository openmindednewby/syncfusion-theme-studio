...
export function repositionFilterPopup(
  ...
  const filterPopup = getVisibleFilterPopup(gridId);
  ...
  const left = resolveFilterPopupLeft(anchorRect, width, preferredOpenToLeft, gridId);
  const top = resolveVerticalPosition(anchorRect, height);

  // Use absolute positioning (matches CSS fallback rule) with scroll offsets.
  // Syncfusion may rewrite inline styles; absolute + scroll is more resilient.
  applyFixedPopupStyle({
    element: filterPopup,
...
export function repositionColumnMenuPopup(
  ...
  const gridId = trigger.closest<HTMLElement>('.e-grid')?.id;
  const popup = getVisibleColumnMenuPopup(gridId);
...
/**
 * Reposition the column chooser dialog so it appears directly below the
 * "Columns" toolbar button, aligned to its left edge.
 *
 * Syncfusion renders the `.e-ccdlg` dialog inside the grid element with
 * `position: static`, which causes the inline `top`/`left` values to be
 * ignored and the dialog to render in document flow far from the toolbar
 * button. This function applies `position: absolute` relative to the grid
 * (which has `position: relative`) and recalculates the correct offsets.
 */
export function repositionColumnChooserPopup(
  gridWrapper: HTMLElement,
): boolean {
  const gridEl = gridWrapper.querySelector<HTMLElement>('.e-grid');
  if (!isValueDefined(gridEl)) return false;

  const dialog = gridEl.querySelector<HTMLElement>('.e-ccdlg.e-popup-open');
  if (!isValueDefined(dialog)) return false;

  const toolbarBtn = gridEl.querySelector<HTMLElement>('.e-cc-toolbar');
  if (!isValueDefined(toolbarBtn)) return false;

  const gridRect = gridEl.getBoundingClientRect();
  const btnRect = toolbarBtn.getBoundingClientRect();

  const topInGrid = btnRect.bottom - gridRect.top;
  const leftInGrid = Math.max(0, btnRect.left - gridRect.left);

  setStyleIfChanged(dialog, 'position', 'absolute');
  setStyleIfChanged(dialog, 'top', `${Math.round(topInGrid)}px`);
  setStyleIfChanged(dialog, 'left', `${Math.round(leftInGrid)}px`);
  setStyleIfChanged(dialog, 'z-index', COLUMN_CHOOSER_Z_INDEX);
  return true;
}
...
/** Check whether a popup element belongs to a specific grid instance. */
function isPopupForGrid(element: HTMLElement, gridId: string): boolean {
  if (element.id.startsWith(gridId)) return true;
  return isValueDefined(element.querySelector(`[id^="${gridId}"]`));
}

function getVisibleColumnMenuPopup(gridId?: string): HTMLElement | undefined {
  ...
  return candidates.find((element) => {
    if (!isPopupVisible(element) || !isColumnMenuPopup(element)) return false;
    if (isValueDefined(gridId) && gridId !== '') return isPopupForGrid(element, gridId);
    return true;
  });
}
...
function getColumnMenuSidePreferenceFromDom(gridId?: string): boolean | undefined {
  const popup = getVisibleColumnMenuPopup(gridId);
  if (!isValueDefined(popup)) return undefined;
...
function getVisibleFilterPopup(gridId?: string): HTMLElement | undefined {
  const selectors =
  ...
  return candidates.reverse().find((element) => {
    if (!isPopupVisible(element)) return false;
    if (isValueDefined(gridId) && gridId !== '') return isPopupForGrid(element, gridId);
    return true;
  });
}
...
function resolveFilterPopupLeft(
  ...
  const effectivePreference =
    preferredOpenToLeft ?? getColumnMenuSidePreferenceFromDom(gridId);
