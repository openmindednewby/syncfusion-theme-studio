...
import { ProgressBarNative, ProgressBarShape, ProgressRingNative } from '@/components/ui/native';
...
const SHARP_SNIPPET = `<ProgressBarNative
  shape={ProgressBarShape.Sharp}
  size="sm"
  value={75}
/>`;

const ROUNDED_SNIPPET = `<ProgressBarNative
  shape={ProgressBarShape.Rounded}
  size="sm"
  value={75}
/>`;

const RING_SNIPPET = `<ProgressRingNative value={66} />`;
          ...
          <div className="space-y-3">
            ...
            {BAR_VALUES.map((v) => (
              <div key={`sharp-${v}`} className="space-y-1">
                <ProgressBarNative shape={ProgressBarShape.Sharp} size="sm" testId={`progress-sharp-${v}`} value={v} />
          ...
          <div className="space-y-3">
            ...
            {BAR_VALUES.map((v) => (
              <div key={`rounded-${v}`} className="space-y-1">
                <ProgressBarNative shape={ProgressBarShape.Rounded} size="sm" testId={`progress-rounded-${v}`} value={v} />
                <span style={LABEL_STYLE}>{v}%</span>
              </div>
            ))}
          </div>
        </div>
        <CopyableCodeSnippet code={SHARP_SNIPPET} />
        <CopyableCodeSnippet code={ROUNDED_SNIPPET} />
      </section>
        ...
        <div className="flex items-end gap-8">
          {RING_VALUES.map((v) => (
            <div key={`ring-${v}`} className="flex flex-col items-center gap-2">
              <ProgressRingNative testId={`progressring-${v}`} value={v} />
              <span style={LABEL_STYLE}>{v}%</span>
            </div>
          ))}
        </div>
        <CopyableCodeSnippet code={RING_SNIPPET} />
      </section>
    </div>
  </div>
);
