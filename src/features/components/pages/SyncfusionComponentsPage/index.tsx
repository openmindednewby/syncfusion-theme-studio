...
import {
  ...
  InputsSection,
  SelectionSection,
} from '../ComponentsPage/sections';

// Lazy load heavy Syncfusion sections to improve initial load performance
const CalendarsSection = lazy(async () =>
  import('../ComponentsPage/sections/CalendarsSection').then((m) => ({ default: m.CalendarsSection }))
);
const NavigationsSection = lazy(async () =>
  import('../ComponentsPage/sections/NavigationsSection').then((m) => ({ default: m.NavigationsSection }))
);
const PopupsSection = lazy(async () =>
  import('../ComponentsPage/sections/PopupsSection').then((m) => ({ default: m.PopupsSection }))
);
...
const SyncfusionComponentsPage = (): JSX.Element => {
  ...
  useEffect(() => {
    loadSyncfusionCss(SyncfusionCssModule.Layouts).catch(() => {});
  }, []);

  return (
    <div className="h-full -m-6" data-testid={TestIds.SYNCFUSION_COMPONENTS_PAGE}>
                ...
                <div className="space-y-8" data-testid={TestIds.SHOWCASE_GRID}>
                  <div>
                    <h2 className="text-2xl font-bold text-text-primary">
                      {FM('components.syncfusionTitle')}
                    </h2>
                    <p className="mt-1 text-text-secondary">
                      {FM('components.syncfusionDescription')}
                    </p>
                  </div>
...
export default SyncfusionComponentsPage;
