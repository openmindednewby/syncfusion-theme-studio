...
describe('errorRegistry', () => {
  ...
  describe('getErrorRules', () => {
    ...
    it('contains all default rules', () => {
      ...
      for (const defaultRule of DEFAULT_ERROR_RULES) expect(ruleNames).toContain(defaultRule.name);
    });
  });
  ...
  describe('isAuthEndpoint', () => {
    ...
    it('returns true for generated mixed-case login endpoint', () => {
    ...
    it('returns true when generated mixed-case auth path is part of a larger URL', () => {
      expect(isAuthEndpoint('https://api.example.com/api/Auth/Login')).toBe(true);
    });
