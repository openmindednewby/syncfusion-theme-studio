...
function handleRedirectEvent(event: ApiEvent, toast: ToastContextValue): void {
  if (event.type !== 'redirect') return;

  if (isValueDefined(event.message) && event.message !== '')
    toast.addToast({
  ...
  if (typeof window !== 'undefined') window.location.href = event.target;
}
  ...
  return function handleEvent(event: ApiEvent): void {
    ...
    switch (event.type) {
      ...
      case 'modal':
        ...
        break;
      case 'redirect':
        handleRedirectEvent(event, toast);
        break;
