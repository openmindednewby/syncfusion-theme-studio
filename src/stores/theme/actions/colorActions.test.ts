...
function createDerivedFromBase(base500: string, isLightMode = true): DerivedComponentColors {
  const palette = generatePaletteFromBase(base500);
  return generateDerivedColors(palette, isLightMode);
}
...
describe('buildDerivedModeConfig', () => {
  ...
  it('cascades disabled colors from palette for light mode', () => {
    ...
    const derived = generateDerivedColors(palette, true);
  ...
  it('cascades disabled colors from palette for dark mode', () => {
    ...
    const derived = generateDerivedColors(palette, false);
...
describe('buildDerivedComponents', () => {
  it('applies derived values to both light and dark configs', () => {
    ...
    const palette = generatePaletteFromBase(PINK_500);
    const result = buildDerivedComponents(components, palette);
  ...
  it('uses mode-specific hover shades for buttons', () => {
    const components = createComponents();
    const palette = generatePaletteFromBase(PINK_500);
    const result = buildDerivedComponents(components, palette);

    expect(result.light.buttons.primary.background).toBe(palette['500']);
    expect(result.light.buttons.primary.backgroundHover).toBe(palette['600']);
    expect(result.dark.buttons.primary.background).toBe(palette['500']);
    expect(result.dark.buttons.primary.backgroundHover).toBe(palette['400']);
  });

  it('cascades disabled colors per mode', () => {
    const components = createComponents();
    const palette = generatePaletteFromBase(PINK_500);
    const result = buildDerivedComponents(components, palette);
...
describe('generateDerivedColors', () => {
  ...
  it('produces button background from primary-500', () => {
    const palette = generatePaletteFromBase(PINK_500);
    const derived = generateDerivedColors(palette, true);

    expect(derived.buttons.primary.background).toBe(palette['500']);
  });

  it('produces sidebar activeBg from primary-500', () => {
    const palette = generatePaletteFromBase(PINK_500);
    const derived = generateDerivedColors(palette, true);

    expect(derived.sidebar.activeBg).toBe(palette['500']);
  });

  it('uses palette-600 for light mode button hover', () => {
    const palette = generatePaletteFromBase(PINK_500);
    const derived = generateDerivedColors(palette, true);

    expect(derived.buttons.primary.backgroundHover).toBe(palette['600']);
  });

  it('uses palette-400 for dark mode button hover', () => {
    const palette = generatePaletteFromBase(PINK_500);
    const derived = generateDerivedColors(palette, false);

    expect(derived.buttons.primary.backgroundHover).toBe(palette['400']);
  });
});
