...
export function buildDerivedComponents(
  ...
  const derivedLight = generateDerivedColors(palette, true);
  const derivedDark = generateDerivedColors(palette, false);
  return {
    light: buildDerivedModeConfig(components.light, derivedLight, palette, true),
    dark: buildDerivedModeConfig(components.dark, derivedDark, palette, false),
  };
}
  ...
  return {
    ...
    updatePrimaryPalette: (baseColor, cascadeToComponents) => {
      ...
      if (cascadeToComponents)
        newTheme = { ...newTheme, components: buildDerivedComponents(newTheme.components, palette) };
