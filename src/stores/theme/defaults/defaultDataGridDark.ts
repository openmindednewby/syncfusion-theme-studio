import type { DataGridConfig } from '../types';

export const DEFAULT_DATAGRID_DARK: DataGridConfig = {
  headerBackground: '31 41 55',
  headerTextColor: '209 213 219',
  headerBorder: '55 65 81',
  rowEvenBackground: '17 24 39',
  rowOddBackground: '31 41 55',
  rowHoverBackground: '55 65 81',
  rowSelectedBackground: '30 58 138',
  cellBorderColor: '55 65 81',
  cellPadding: '12px 16px',
  paginationBackground: '17 24 39',
  toolbarBackground: '31 41 55',
  toolbarTextColor: '209 213 219',
  toolbarBorderColor: '55 65 81',
  filterRowBackground: '17 24 39',
  filterRowBorderColor: '55 65 81',
  filterInputBackground: '31 41 55',
  groupHeaderBackground: '55 65 81',
  groupHeaderTextColor: '209 213 219',
  groupDropAreaBackground: '31 41 55',
  groupDropAreaTextColor: '156 163 175',
  groupDropAreaBorderColor: '55 65 81',
  groupChipBackground: '55 65 81',
  groupChipTextColor: '209 213 219',
  footerBackground: '31 41 55',
  footerTextColor: '209 213 219',
  footerBorderWidth: '2px',
  footerFontSize: '13px',
  footerFontWeight: '600',
  footerPadding: '10px 16px',
  editCellBackground: '31 41 55',
  editCellBorderColor: '96 165 250',
  editDirtyIndicatorColor: '251 191 36',
  rowSelectedTextColor: '249 250 251',
  cellSelectedBackground: '30 58 138',
  sortIconColor: '156 163 175',
  resizeHandleColor: '75 85 99',
  paginationTextColor: '209 213 219',
  paginationActiveBackground: '96 165 250',
  paginationActiveTextColor: '255 255 255',
  paginationHoverBackground: '55 65 81',
  paginationBorderColor: '55 65 81',
  pagerContainerBorderColor: '55 65 81',
  paginationDefaultPageSize: 10,
  paginationPageSizeOptions: '10,20,50,100',
  paginationNavColor: '156 163 175',
  paginationNavDisabledColor: '75 85 99',
  paginationInfoTextColor: '156 163 175',
  actionButtonColor: '96 165 250',
  actionButtonHoverColor: '147 197 253',
  detailRowBackground: '31 41 55',
  dragHandleColor: '107 114 128',
  defaultTextAlign: 'center',
  headerTextAlign: 'center',
  headerTextPadding: '0px 0px 0px 34px',
  defaultVerticalAlign: 'middle',
  headerVerticalAlign: 'middle',
  rowHeight: '42px',
  columnMenuBackground: '31 41 55',
  columnMenuTextColor: '209 213 219',
  columnMenuBorderColor: '55 65 81',
  columnMenuHoverBackground: '55 65 81',
  columnMenuSeparatorColor: '55 65 81',
  columnMenuTriggerColor: '156 163 175',
  columnMenuTriggerHoverColor: '209 213 219',
  headerTextTransform: 'uppercase',
  headerFontSize: '14px',
  headerFontWeight: '600',
  headerLetterSpacing: '0',
  selectionBarBackground: '31 41 55',
  selectionBarChipBackground: '67 129 145',
  selectionBarChipTextColor: '246 246 246',
  selectionBarClearBackground: '55 65 81',
  selectionBarClearTextColor: '209 213 219',
  radioColor: '67 129 145',
};
