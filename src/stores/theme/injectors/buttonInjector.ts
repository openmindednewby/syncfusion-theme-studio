...
export function injectSplitButtonVariables(root: HTMLElement, c: ComponentConfigSingle): void {
  ...
  root.style.setProperty('--component-split-btn-border-radius', cssBorderRadius(s.borderRadius));
...
function injectButtonVariantVars(root: HTMLElement, c: ComponentConfigSingle): void {
  for (const v of VARIANTS) {
    ...
    root.style.setProperty(`${prefix}-text-active`, cssColor(btn.textColorActive));
    root.style.setProperty(`${prefix}-border`, cssColor(btn.borderColor));
    root.style.setProperty(`${prefix}-border-active`, cssColor(btn.borderColorActive));
...
const ICON_BTN_VARIANTS = ['primary', 'secondary', 'tertiary', 'outline'] as const;
