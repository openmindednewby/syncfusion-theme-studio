...
import { injectAlertBadgeVariables, injectAvatarVariables, injectExternalLinkVariables, injectImageVariables, injectLoaderVariables, injectProgressBarVariables, injectSearchPanelVariables, injectSkeletonLoaderVariables, injectSliderVariables, injectTabsVariables, injectTextDescriptionVariables, injectTimelineVariables, injectTooltipVariables } from './dataDisplayInjector';
import { injectDataGridVariables } from './dataGridInjector';
import { injectAlertVariables, injectChipVariables, injectDatePickerVariables, injectDialogVariables, injectErrorMessageVariables, injectFlexBoxVariables, injectInputMessageVariables, injectMessageVariables, injectSelectVariables, injectToastVariables } from './feedbackInjector';
import { injectCheckboxVariables, injectRadioVariables, injectToggleVariables } from './formControlInjector';
import { injectGridButtonVariables } from './gridButtonInjector';
import { injectGridDropdownVariables } from './gridDropdownInjector';
...
export { injectAlertVariables, injectChipVariables, injectDatePickerVariables, injectDialogVariables, injectErrorMessageVariables, injectFlexBoxVariables, injectInputMessageVariables, injectMessageVariables, injectSelectVariables, injectToastVariables };
export { injectCheckboxVariables, injectRadioVariables, injectToggleVariables };
export { injectGridButtonVariables };
export { injectGridDropdownVariables };
...
export { injectAlertBadgeVariables, injectAvatarVariables, injectExternalLinkVariables, injectImageVariables, injectLoaderVariables, injectProgressBarVariables, injectSearchPanelVariables, injectSkeletonLoaderVariables, injectSliderVariables, injectTabsVariables, injectTextDescriptionVariables, injectTimelineVariables, injectTooltipVariables };
...
export function injectSidebarVariables(root: HTMLElement, c: ComponentConfigSingle): void {
  root.style.setProperty('--component-sidebar-background', cssColor(c.sidebar.background));
  root.style.setProperty('--component-sidebar-text-color', cssColor(c.sidebar.textColor));
  root.style.setProperty('--component-sidebar-active-bg', cssColor(c.sidebar.activeItemBackground));
  root.style.setProperty('--component-sidebar-active-text', cssColor(c.sidebar.activeItemTextColor));
  root.style.setProperty('--component-sidebar-hover-bg', cssColor(c.sidebar.hoverItemBackground));
  root.style.setProperty('--component-sidebar-border-right', cssColor(c.sidebar.borderRight));
  ...
  root.style.setProperty('--component-sidebar-font-family', c.sidebar.fontFamily);
  root.style.setProperty('--component-sidebar-item-border-radius', c.sidebar.itemBorderRadius);
  root.style.setProperty('--component-sidebar-menu-button-padding-left', c.sidebar.menuButtonPaddingLeft);
  ...
  root.style.setProperty('--component-sidebar-highlight-color', cssColor(c.sidebar.searchHighlightColor));
  root.style.setProperty('--component-sidebar-highlight-scale', c.sidebar.searchHighlightScale);

  // Search input variables
  root.style.setProperty('--component-sidebar-search-bg', cssColor(c.sidebar.searchBackground));
  root.style.setProperty('--component-sidebar-search-border', cssColor(c.sidebar.searchBorder));
  root.style.setProperty('--component-sidebar-search-radius', c.sidebar.searchBorderRadius);
  root.style.setProperty('--component-sidebar-search-text', cssColor(c.sidebar.searchTextColor));
  root.style.setProperty('--component-sidebar-search-placeholder', cssColor(c.sidebar.searchPlaceholderColor));
  root.style.setProperty('--component-sidebar-search-focus-bg', cssColor(c.sidebar.searchFocusBackground));
  root.style.setProperty('--component-sidebar-search-focus-border', cssColor(c.sidebar.searchFocusBorder));
...
export function injectComponentVariables(root: HTMLElement, components: ComponentConfigSingle): void {
  ...
  injectGridButtonVariables(root, components);
  injectGridDropdownVariables(root, components);
  ...
  injectLoaderVariables(root, components);
  injectSkeletonLoaderVariables(root, components);
  injectSliderVariables(root, components);
}
...
function injectDisplayVariables(root: HTMLElement, c: ComponentConfigSingle): void {
  ...
  injectImageVariables(root, c);
  injectTextDescriptionVariables(root, c);
  injectSearchPanelVariables(root, c);
}
...
function injectFeedbackVariables(root: HTMLElement, c: ComponentConfigSingle): void {
  ...
  injectInputMessageVariables(root, c);
...
/** Wrap a raw RGB triplet in `rgb()`, but pass through values already in rgb/rgba/hex format. */
function cssColor(value: string): string {
  return value.startsWith('rgb') || value.startsWith('#') ? value : `rgb(${value})`;
}
