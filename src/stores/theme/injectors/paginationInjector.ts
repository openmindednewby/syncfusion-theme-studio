...
export function injectPaginationVariables(root: HTMLElement, c: ComponentConfigSingle): void {
  ...
  if (p.buttonBorderColor !== '') root.style.setProperty('--component-pagination-btn-border-color', `rgb(${p.buttonBorderColor})`);
  if (p.buttonBorderWidth !== '') root.style.setProperty('--component-pagination-btn-border-width', p.buttonBorderWidth);
  if (p.buttonFontSize !== '') root.style.setProperty('--component-pagination-btn-font-size', p.buttonFontSize);
  if (p.buttonSize !== '') root.style.setProperty('--component-pagination-btn-size', p.buttonSize);
