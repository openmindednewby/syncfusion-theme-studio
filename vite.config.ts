...
export default defineConfig(({ mode }: ConfigEnv) => {
  ...
  // process.env takes precedence (allows Tilt env overrides), then .env file, then default
  const backendUrl = process.env['VITE_BACKEND_URL'] || env['VITE_BACKEND_URL'] || 'http://localhost:5150';
  ...
  const pwaPlugin = VitePWA({
    ...
    workbox: {
      ...
      // Replace the old SW immediately on each deploy. Without these three
      // flags, users keep a stale SW whose precache manifest points at asset
      // hashes from the previous build — every one of those fetches 404s.
      skipWaiting: true,
      clientsClaim: true,
      ...
      resolveDependencies: (filename, deps) => {
        ...
        return deps.filter((dep) => {
          const isHeavyChunk =
            ...
            dep.includes('syncfusion-diagrams');
          return !isHeavyChunk;
        });
      },
    },
    ...
    rollupOptions: {
      output: {
        ...
        manualChunks: (id) => {
          ...
          // Syncfusion Diagrams - very large
          if (id.includes('ej2-diagrams') || id.includes('ej2-react-diagrams')) {
            return 'syncfusion-diagrams';
          }
          // Syncfusion base
  ...
  server: {
    ...
    warmup: {
      clientFiles: [
        ...
        './src/localization/utils/i18n.ts',
        './src/components/common/components/LoadingSpinner.tsx',
